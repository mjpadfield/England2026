...
export default function Home() {
  return (
    <main className="min-h-screen">
      ...
      >
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          ...
          <h1 className="text-4xl md:text-7xl font-black text-white tracking-tight mb-4 text-shadow">
            England
            <span className="text-england-red"> 2026</span>
          </h1>
          <p className="text-lg md:text-2xl text-white/70 mb-2 font-light">
            The World Cup Tour
          </p>
          <p className="text-white/40 text-sm md:text-base tracking-widest uppercase mb-12">
          ...
          <div className="flex flex-wrap justify-center gap-3 mb-12">
            {[
              ...
              { label: "Round of 32", sub: "2 Jul · Mercedes-Benz, ATL" },
              { label: "Round of 16", sub: "6 Jul · Azteca, CDMX" },
              { label: "Quarterfinal", sub: "11 Jul · Hard Rock, MIA" },
            ].map((m) => (
              <div
                key={m.label}
                className="bg-england-red/10 border border-england-red/30 rounded-xl px-4 py-3 text-left min-w-[140px]"
              >
                <p className="text-england-red text-xs font-bold uppercase tracking-wide">
                  {m.label}
                </p>
          ...
          <a
            href="#itinerary"
            className="inline-flex items-center gap-2 bg-england-red hover:bg-england-red/80 text-white font-semibold px-8 py-3 rounded-full transition-colors"
          >
            View Full Itinerary
            <span>↓</span>
          </a>
      ...
      {/* Itinerary timeline */}
      <section id="itinerary" className="py-20 px-4 border-t border-white/5">
                  ...
                  <span
                    className={`absolute -left-8 top-1.5 w-3 h-3 rounded-full border-2 ${
                      item.event.startsWith("⚽")
                        ? "bg-england-red border-england-red"
